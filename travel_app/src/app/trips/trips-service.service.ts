...
export class TripsServiceService {
  ...
  getTravelsByParametars(destination: string, price: any, travel_type: string): Array<TripsModel> {
    ...
    TripsServiceService.travels.map((travel: TripsModel) => {
      ...
      if (travel.destination.toString().toLowerCase().includes(destination.toLowerCase())) {
        if (travel.travel_type.toString().toLowerCase().includes(travel_type.toLowerCase())) {
          if (travel.price >= price - 1000 && travel.price <= price) {
            fastFilteredTrips.push(travel);
          }
        }
      }
